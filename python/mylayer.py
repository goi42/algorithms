class mylayer:
    def __init__(self,element,name=None):#,compared=False):
        if not isinstance(element,list):
            raise TypeError('element is a '+type(element)+', not a list')
        if not (element or name):
            raise IOError('cannot pass empty list without passing a name')
