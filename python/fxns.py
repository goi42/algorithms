...
class unc:
    "handle statistical and systematic uncertainties using the uncertainties package"
    
    def __init__(self, value, statistical=0, systematic=0):
        import uncertainties
        
        self.statistical = uncertainties.ufloat(value, statistical, 'statistical')
        self.systematic = uncertainties.ufloat(value, systematic, 'systematic')
    
    def getValue(self):
        assert self.statistical.n == self.systematic.n
        return self.statistical.n
    
    def getCombined(self):
        import uncertainties
        from uncertainties import umath
        
        return uncertainties.ufloat(self.getValue(), umath.sqrt(self.statistical.s**2 + self.systematic.s**2), 'combined')
    
    def getError(self, typ=None):
        if typ is None:
            return self.getCombined().s
        elif typ == 'statistical':
            return self.statistical.s
        elif typ == 'systematic':
            return self.systematic.s
        else:
            raise ValueError('typ {0} not recognized'.format(typ))
        
    def __add__(self, another):
        new = unc(0)
        new.statistical = self.statistical + another.statistical
        new.systematic = self.systematic + another.systematic
        return new
