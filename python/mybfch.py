...
class mybfch: #abstract base class for branch and fch classes
    def __init__(self,c=None):
        self.c = [] #cuts to be applied
        if c:
            if isinstance(c,list) and all(isinstance(x,mycut) for x in c):
                self.c=c
            else:
                raise TypeError('passed a '+c.__class__.__name__+' as "c" instead of a list.')
    def add_cut(self,*args,**kwargs):
        if isinstance(args[0],mycut):
            ...
            elif all(isinstance(x,mycut) for x in args): # if it's a bunch of cuts
                self.c.extend(args) # just add them
            else:
                raise TypeError
        elif all(isinstance(x, list) for x in args): # if it's a list (or lists)
            for ar in args:
                if all(isinstance(x,mycut) for x in ar): # of cuts, add them
                    self.c.extend(ar)
            ...
            self.c.append(mycut(*args,**kwargs)) # create new cut, append it
